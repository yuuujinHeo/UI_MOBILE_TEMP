...
void MapView::loadFile(QString name, QString type){
    ...
    file_path = QDir::homePath() + "/maps/"+name + "/map_edited.png";
    if(QFile::exists(file_path)){
    ...
    file_path = QDir::homePath() + "/maps/"+name + "/map_velocity.png";
    if(QFile::exists(file_path)){
...
void MapView::saveRotateMap(){
    ...
    plog->write("[Annotation] Map Size Cut : "+QString().sprintf("%f,%f ~ %f,%f",cut_box[0].x,cut_box[0].y,cut_box[1].x,cut_box[1].y));
